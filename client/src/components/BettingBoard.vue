...
const getBetsForColor = (color) => {
    return props.bets.filter(bet => {
        if (color === 'red') {
            return (bet.type === 'color' && bet.value === 'red') || (bet.type === 'number' && getNumbersByColor('red').includes(bet.value));
        } else if (color === 'green') {
            return (bet.type === 'number' && bet.value === 0) || (bet.type === 'type' && ['even', 'odd'].includes(bet.value));
        } else if (color === 'black') {
            return (bet.type === 'color' && bet.value === 'black') || (bet.type === 'number' && getNumbersByColor('black').includes(bet.value));
        }
        return false;
    });
};
...
const getSectionClass = (color) => {
    ...
    if (props.lastResult.color === color) {
        // Winner
        const borderColor = color === 'red' ? 'border-primary' : color === 'green' ? 'border-success' : 'border-surface-lighter';
        const shadowColor = color === 'red' ? 'rgba(255,62,62,0.3)' : color === 'green' ? 'rgba(0,230,118,0.3)' : 'rgba(255,255,255,0.1)';
...
const getNumberClass = (num) => {
    if (!props.lastResult) return 'bg-surface-light hover:bg-surface-lighter border-white/5';
