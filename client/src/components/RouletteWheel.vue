<template>
  <div class="relative w-full max-w-4xl mx-auto flex justify-center items-end mb-4 sm:mb-8">
    <!-- Wheel Container -->
    <div class="relative w-full aspect-square max-w-[600px]">
      <div 
        class="w-full h-full rounded-full transition-transform cubic-bezier(0.1, 0.8, 0.1, 1) transform-gpu"
      ...
      <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
          ...
          <div v-if="status.includes('ROLLING IN')" class="text-center">
              <div class="text-4xl sm:text-6xl font-outfit font-thin md:font-light bg-gradient-to-r from-white to-white/40 bg-clip-text text-transparent mb-2 tracking-tighter drop-shadow-2xl">
                  {{ timeLeft.toFixed(2) }}
              </div>
              <div class="text-text-muted text-[10px] sm:text-sm font-thin md:font-light tracking-[0.3em] uppercase">Rolling In</div>
          </div>
          ...
          <div v-else-if="lastResult" class="text-center animate-fade-in">
              ...
              <div :class="[
                  'text-5xl sm:text-7xl font-outfit font-thin md:font-light mb-2 sm:mb-4 drop-shadow-2xl',
                  lastResult.color === COLORS.RED ? 'text-primary' : lastResult.color === COLORS.GREEN ? 'text-success' : 'text-white'
              ]">
                  {{ lastResult.number }}
              </div>
              <div :class="[
                  'text-[10px] sm:text-sm font-bold uppercase tracking-widest px-3 py-1 sm:px-4 sm:py-1.5 rounded-full inline-block border font-outfit',
          ...
          <div v-else class="text-center">
              <div class="text-xl sm:text-3xl font-outfit font-thin md:font-light tracking-[0.3em] text-white uppercase animate-pulse">Rolling</div>
              <div class="flex justify-center mt-2 sm:mt-4 space-x-2">
      ...
      <!-- Pointer -->
      <div class="absolute -bottom-1 left-1/2 -translate-x-1/2 z-30">
          <div class="w-0 h-0 border-l-[10px] sm:border-l-[12px] border-l-transparent border-r-[10px] sm:border-r-[12px] border-r-transparent border-b-[20px] sm:border-b-[24px] border-b-accent filter drop-shadow-[0_0_15px_rgba(255,215,0,0.6)]"></div>
      </div>
