...
type SupportedBetType = 'color' | 'number' | 'type';

const validateBet = (
  type: SupportedBetType | '' | 'invalid' | null | undefined,
  value: number | string | null | undefined,
  amount: number | null | undefined
): string | null => {
  if (amount === null || amount === undefined || Number.isNaN(amount) || amount < BET_LIMITS.MIN || !Number.isInteger(amount)) {
    return `Invalid bet amount (minimum is ${BET_LIMITS.MIN})`;
  }

  const validTypes: readonly SupportedBetType[] = ['number', 'color', 'type'];
  if (type === null || type === undefined || !validTypes.includes(type as SupportedBetType)) {
    return 'Invalid bet type';
  }

  if (type === 'number') {
    if (typeof value !== 'number' || !Number.isInteger(value) || value < NUMBER_RANGE.MIN || value > NUMBER_RANGE.MAX) {
      return `Invalid number bet (must be ${NUMBER_RANGE.MIN}-${NUMBER_RANGE.MAX})`;
    }
  } else if (type === 'color') {
    if (!Object.values(COLORS).includes(value as (typeof COLORS)[keyof typeof COLORS])) {
      return 'Invalid color bet';
    }
  } else if (type === 'type') {
    if (!Object.values(BET_TYPES).includes(value as (typeof BET_TYPES)[keyof typeof BET_TYPES])) {
...
describe('Game Constants', () => {
  describe('SEGMENTS', () => {
    ...
    it('should have green only for number 0', () => {
      const greenSegments = SEGMENTS.filter(segment => segment.color === 'green');
      expect(greenSegments).toHaveLength(1);
      expect(greenSegments[0].number).toBe(0);
    });

    it('should have balanced red and black segments', () => {
      const redCount = SEGMENTS.filter(segment => segment.color === 'red').length;
      const blackCount = SEGMENTS.filter(segment => segment.color === 'black').length;
...
describe('Bet Validation Logic', () => {
  describe('Amount validation', () => {
    ...
    it('should reject NaN amount', () => {
      expect(validateBet('number', 5, Number.NaN)).toBe(`Invalid bet amount (minimum is ${BET_LIMITS.MIN})`);
    });
    ...
    it('should accept large amounts', () => {
      expect(validateBet('number', 5, 1_000_000)).toBeNull();
    });
  });
  ...
  describe('Number bets', () => {
    it('should accept numbers within valid range', () => {
      for (let number = NUMBER_RANGE.MIN; number <= NUMBER_RANGE.MAX; number += 1) {
        expect(validateBet('number', number, 100)).toBeNull();
      }
    });

    it('should reject number above max', () => {
      expect(validateBet('number', NUMBER_RANGE.MAX + 1, 100)).toBe(
        `Invalid number bet (must be ${NUMBER_RANGE.MIN}-${NUMBER_RANGE.MAX})`
      );
    });
